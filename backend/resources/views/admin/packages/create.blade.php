    ...
    <form method="POST" action="{{ route('admin.packages.store') }}">
        ...
        <div class="grid grid-2">
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="Contoh: Paket 6 Bulan Pro">
                @error('name') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            <div class="form-group">
                <label class="form-label">Slug (otomatis jika kosong)</label>
                <input type="text" name="slug" class="form-control" value="{{ old('slug') }}" placeholder="paket-6-bulan-pro">
        ...
        <div class="form-group" style="background: var(--surface); padding: 20px; border-radius: 12px; border: 1px solid var(--divider);">
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 16px;">
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 500;">
                    <input type="radio" name="validity_type" value="duration" {{ old('validity_type', 'duration') == 'duration' ? 'checked' : '' }} onchange="toggleValidityFields()">
                    <span>Durasi (Pilihan Bulan / Tahun / Hari)</span>
                </label>
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 500;">
                    <input type="radio" name="validity_type" value="date_range" {{ old('validity_type') == 'date_range' ? 'checked' : '' }} onchange="toggleValidityFields()">
                    <span>Range Waktu (Dari Tanggal s/d Tanggal)</span>
                </label>
                <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 500;">
                    <input type="radio" name="validity_type" value="fixed_date" {{ old('validity_type') == 'fixed_date' ? 'checked' : '' }} onchange="toggleValidityFields()">
            ...
            <div id="section_duration" class="validity-section" style="margin-top: 12px;">
                ...
                <div class="form-group" style="max-width: 300px;">
                    ...
                    <input type="number" id="default_duration_days" name="default_duration_days" class="form-control" value="{{ old('default_duration_days', 30) }}" min="1" placeholder="90">
            ...
            <div id="section_date_range" class="validity-section" style="margin-top: 12px; display: none;">
                <div class="grid grid-2">
                    <div class="form-group">
                        ...
                        <input type="date" id="start_date" name="start_date" class="form-control" value="{{ old('start_date', date('Y-m-d')) }}">
                        @error('start_date') <div class="form-error">{{ $message }}</div> @enderror
                    </div>
                    <div class="form-group">
                        <label class="form-label">Tanggal Selesai *</label>
                        <input type="date" id="end_date_range" name="end_date" class="form-control" value="{{ old('end_date') }}">
            ...
            <div id="section_fixed_date" class="validity-section" style="margin-top: 12px; display: none;">
                <div class="form-group" style="max-width: 300px;">
                    ...
                    <input type="date" id="end_date_fixed" name="end_date" class="form-control" value="{{ old('end_date') }}">
                    @error('end_date') <div class="form-error">{{ $message }}</div> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="description" class="form-control" rows="3" placeholder="Keterangan singkat mengenai paket ini">{{ old('description') }}</textarea>
        </div>
