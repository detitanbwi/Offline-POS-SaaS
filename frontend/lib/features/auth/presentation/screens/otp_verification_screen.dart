...
import 'new_pin_setup_screen.dart';
...
class _OtpVerificationScreenState extends ConsumerState<OtpVerificationScreen> {
  ...
  Future<void> _verifyOtp(String otpCode) async {
    ...
    if (success) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => NewPinSetupScreen(email: widget.email),
        ),
      );
    } else {
  ...
  Future<void> _resendOtp() async {
    final success = await ref.read(pinRecoveryProvider.notifier).requestOtp(widget.email);
    if (!mounted) return;
